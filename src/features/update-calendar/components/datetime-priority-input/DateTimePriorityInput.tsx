...
const DateTimePriorityInput: React.FC<IEventPickerInput> = (props: IEventPickerInput) => {
  ...
  const displayStart = dayjs(start).format(appConstants.FORMAT_DATE_TIME.HH_MM_DD_MM_YYYY);
  const displayEnd = dayjs(end).format(appConstants.FORMAT_DATE_TIME.HH_MM_DD_MM_YYYY);
  ...
  return (
    <AppView>
      ...
      <AppHelperText type="error" visible={props.dateError.isError} children={props.dateError.desc} />
    </AppView>
  );
};
