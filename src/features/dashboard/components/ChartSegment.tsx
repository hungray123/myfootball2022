...
import { MapCommonItemChart } from 'app/type';
...
const ChartSegment: React.FC = () => {
  ...
  const data = useAppSelector(state => state.dashboardState.dataSegment);
  const listValue = useAppSelector(state => state.dashboardState.dataValueSegment);
  const commonData = listCommonByCode(appConstants.COMMON_CODE.SEGMENT_CUSTOMER_CONFIG) as Array<MapCommonItemChart>;
