...
const ChartStatus: React.FC = () => {
  ...
  const data = useAppSelector(state => state.dashboardState.dataStatus);
  const listValue = useAppSelector(state => state.dashboardState.dataValueStatus);
  const commonData = listCommonByCode(appConstants.COMMON_CODE.CUSTOMER_STATUS) as Array<MapCommonItemChart>;
