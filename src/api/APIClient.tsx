...
import { apiServiceConfig, authenConfig, config, keycloakConfig } from 'constants/config';
import uuid from 'react-native-uuid';
import { createAppError, handleException, translate, urlEncodedBody } from 'utils';
...
const DEFAULT_NUMBER_RETRY_REFRESH_TOKEN = apiServiceConfig.NUMBER_RETRY_REFRESH_TOKEN; //milisecond
...
class ApiClient {
  ...
  setAuthorization(token: string, tokenType: string, token_expires_in: number, refresh_token: string) {
  ...
  actionRefreshToken() {
    ...
    if (this.tokenTimeRemaining > timeToRefresh) {
      ...
      console.log('Thực hiện retry refresh token lần ', this.numberRefreshToken);
      const body = {
      ...
      apiClient
        ...
        .then(response => {
          this.isRefreshingToken = false;
          if (this.tokenTimeRemaining > 0 && this.numberRefreshToken <= DEFAULT_NUMBER_RETRY_REFRESH_TOKEN) {
            const { access_token, token_type, refresh_token, expires_in } = response;
            this.setAuthorization(access_token, token_type, expires_in, refresh_token);
          } else {
  ...
  get<T>(url: string, params?: AppApiParams, apiConfig?: AppApiRequestConfig): Promise<T> {
    return new Promise((resolve, reject) => {
      const source = CancelToken.source();
      const timeoutValue = apiConfig?.timeout ? apiConfig.timeout : DEFAULT_TIMEOUT;
  ...
  post<T>(url: string, data?: AppApiParams, apiConfig?: AppApiRequestConfig): Promise<T> {
  ...
  put<T>(url: string, data?: AppApiParams, apiConfig?: AppApiRequestConfig): Promise<T> {
  ...
  delete<T>(url: string, params?: AppApiParams, apiConfig?: AppApiRequestConfig): Promise<T> {
